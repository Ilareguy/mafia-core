...
#ifndef DEF_MAFIA_CORE_VECTOR_H
#define DEF_MAFIA_CORE_VECTOR_H
...
namespace mafia
{
    ...
    template<class TY>
    //Copy from STL headers. Because GCC6 doesn't support it
    constexpr const TY& clamp(
            const TY& val, const TY& min_val,
            const TY& max_val
    )
    ...
    public:
        ...
        constexpr vector3_base& operator=(const vector3_base& other) noexcept
        {
            x = other.x;
            y = other.y;
            z = other.z;
        ...
        constexpr vector3_base operator*(const T& val) const noexcept
        {
            return vector3_base(
                    x * val, y * val, z * val
            );
        ...
        {
            T invVal = T(1) / val;
            return vector3_base(x * invVal, y * invVal, z * invVal);
        }

        constexpr vector3_base operator+(const vector3_base& v) const noexcept
        {
            return vector3_base(
                    x + v.x, y + v.y, z + v.z
            );
        ...
        constexpr vector3_base& operator*=(const vector3_base& v) noexcept
        {
            x *= v.x;
            y *= v.y;
            z *= v.z;
        ...
        constexpr vector3_base& operator*=(T mag) noexcept
        {
            x *= mag;
            y *= mag;
            z *= mag;
        ...
        constexpr bool operator==(const vector3_base& r) const noexcept
        {
        ...
        constexpr T distance(const vector3_base& v) const noexcept
        {
            vector3_base dist = (*this - v);
            dist = dist * dist;
            return std::sqrt(dist.x + dist.y + dist.z);
        }

        constexpr T distance_squared(const vector3_base& v) const noexcept
        {
            vector3_base dist = (*this - v);
            dist = dist * dist;
        ...
        constexpr T distance_2d(const vector3_base& v) const noexcept
        {
            vector3_base dist = (*this - v);
            dist = dist * dist;
            return std::sqrt(dist.x + dist.y);
        }

        constexpr T distance_2d_squared(const vector3_base& v) const noexcept
        {
            vector3_base dist = (*this - v);
            dist = dist * dist;
        ...
        constexpr vector3_base cross(const vector3_base& v) const noexcept
        {
            return vector3_base(
        ...
        constexpr bool zero_distance() const noexcept
        {
            return (x == 0.0f && y == 0.0f && z == 0.0f);
        }

        static constexpr vector3_base
        lerp(const vector3_base& A, const vector3_base& B, const T t) noexcept { return A * t + B * (1.f - t); }

        /// @brief linear interpolate
        constexpr vector3_base lerp(const vector3_base& B, const T t) noexcept
        {
            return vector3_base::lerp(
                    *this, B, t
            );
        }
    ...
    public:
        ...
        constexpr vector2_base& operator=(const vector2_base& other) noexcept
        {
            x = other.x;
            y = other.y;
            return *this;
        ...
        constexpr vector2_base operator/(const T& val) const noexcept
        {
            T invVal = T(1) / val;
            return vector2_base(x * invVal, y * invVal);
        }

        constexpr vector2_base operator+(const vector2_base& v) const noexcept
        {
            return vector2_base(
                    x + v.x, y + v.y
            );
        ...
        constexpr vector2_base& operator*=(const vector2_base& v) noexcept
        {
            x *= v.x;
            y *= v.y;
            return *this;
        ...
        constexpr vector2_base& operator*=(T mag) noexcept
        {
            x *= mag;
            y *= mag;
            return *this;
        ...
        constexpr bool operator>(const vector2_base& r) const noexcept
        {
            if (*this == r)
            { return false; }
            return magnitude() > r.magnitude();
        ...
        constexpr T distance(const vector2_base& v) const noexcept
        {
            vector2_base dist = (*this - v);
            dist = dist * dist;
            return std::sqrt(dist.x + dist.y);
        }

        constexpr T distance_squared(const vector2_base& v) const noexcept
        {
            vector2_base dist = (*this - v);
            dist = dist * dist;
            return (dist.x + dist.y);
        ...
        /// @brief linear interpolate
        static constexpr vector2_base
        lerp(const vector2_base& A, const vector2_base& B, const T t) noexcept { return A * t + B * (1.f - t); }

        /// @brief linear interpolate
        constexpr vector2_base lerp(const vector2_base& B, const T t) const noexcept
        {
            return vector2_base::lerp(
                    *this, B, t
            );
        }
