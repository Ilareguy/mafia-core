...
uintptr_t mafia::memory_utility::_private::find_game_state(uintptr_t stack_base)
{
    auto checkValid = [](uintptr_t base) -> bool {
        ...
        if (ptr_check(reinterpret_cast<void*>(size_check_type->p1)))
        { return false; }
        if (ptr_check(reinterpret_cast<void*>(size_check_type->p2)))
        { return false; }
        if (ptr_check(reinterpret_cast<void*>(size_check_type->p3)))
        { return false; }
        if (ptr_check(reinterpret_cast<void*>(size_check_type->p4)))
        { return false; }
        return true;
    };
...
void mafia::memory_utility::init(uintptr_t stack_base)
{
    ...
    auto future_allocatorVtablePtr = std::async(
            std::launch::deferred, [&]() {
                ...
                return (_private::find_in_memory(
                        baseAddress,
                        moduleSize,
